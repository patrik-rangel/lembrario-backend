...
func main() {
	log.Println("Opa! Worker iniciando...")
	...
	go func() {
		sig := <-sigChan
		log.Printf("Opa! Recebido sinal %v, iniciando shutdown graceful...", sig)
		cancel()
	}()

	// 7. Iniciar o worker
	log.Println("Show! Worker iniciado, aguardando mensagens...")
	worker.StartWorker(ctx, redisClient, queries, searchClient)

	log.Println("Tchau! Worker finalizado")
}
